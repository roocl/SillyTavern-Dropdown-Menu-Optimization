...
- `st-dropdown-frequency-sorter.user.js`
  ...
  - 清空和查看统计使用紧凑小方形按钮，避免扩展设置栏变窄时按钮文字竖排挤占空间。
  - 查看统计会展示对应类别的点击次数排行。
...
世界书菜单使用 jQuery/select2。只监听原生 `change` 不足以覆盖 SillyTavern 的世界书交互，因为世界书代码会使用 `$('#world_editor_select').val(index).trigger('change')`，这是 jQuery 合成事件。因此脚本必须额外绑定：
...
select2 用户选择可能先触发 `select2:select`，再触发 `change`。需要用短时间门闩忽略同一 select 紧随其后的 `change`，避免一次交互被重复统计。
